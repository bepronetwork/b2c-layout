...
function getGames() {
  return Cache.getFromCache("appInfo")
    ? Cache.getFromCache("appInfo").games
    : [];
}
...
async function processResponse(response) {
  try {
    ...
    await store.dispatch(
      setMessageNotification(new String(err.message).toString())
    );
    throw err;
  }
}
...
function loadFakeBets(rows, games, size) {
  ...
  do {
    i++;
    let payout = Math.random() * (2.0 - 0.05) + 0.05;
    let winAmount = Math.random() * (2.0 - 0.05) + 0.05;
  ...
  if (game) {
    const newRow = {
      ...
      username:
        fakeUserName.length > 10
          ? fakeUserName.substring(0, 4) +
            "..." +
...
function getAddOn() {
  return Cache.getFromCache("appInfo")
    ? Cache.getFromCache("appInfo").addOn
    : {};
}
...
function formatOpponentData(match, index, gameImage) {
  ...
  const oddType = _.isEmpty(match.odds)
    ? []
...
function getIcon(index) {
  const { icons } = Cache.getFromCache("appInfo")
    ? Cache.getFromCache("appInfo").customization
    : {};
