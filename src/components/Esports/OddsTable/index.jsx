...
class OddsTable extends Component {
  ...
  projectData = async (props) => {
    ...
    const activeMarket = match.market
      ? match.market.status === "active"
      : false;
  ...
  render() {
    ...
    const drawId = parseInt(
      opponent1.odd.participant_id + "" + opponent2.odd.participant_id,
    );
    ...
    const opponent1Bet = formatOpponentBet(
      opponent1,
      matchId,
      matchName,
      0,
    ...
    const opponent2Bet = formatOpponentBet(
      opponent2,
      matchId,
      matchName,
      0,
    ...
    const drawBet =
      drawOdd !== null
        ? formatDrawBet(drawId, drawOdd, matchId, matchName, gameImage, 0, id)
        : null;
      ...
      <div styleName="bets-menu">
        <div>
          ...
          <div styleName={mainStyles}>
            <div
              styleName={opponent1Styles}
              onClick={() =>
                isOpponent1Selected
                  ? this.handleRemoveToBetSlip(opponent1.id)
                  : this.handleAddToBetSlip(opponent1Bet)
              }
              style={activeMarket ? buttonStyles.open : buttonStyles.locked}
            >
              ...
              <span styleName="group left">
                {opponent1.status !== "stable" && (
                  <div styleName={`arrow ${opponent1.status}`} />
                )}{" "}
                <Typography variant={"x-small-body"} color={"white"}>
                  {opponent1.odd.odd}
                </Typography>
            ...
            {drawOdd !== null ? (
              <div
                ...
                onClick={() =>
                  isDrawSelected
                    ? this.handleRemoveToBetSlip(drawId)
                    : this.handleAddToBetSlip(drawBet)
                }
                style={activeMarket ? buttonStyles.open : buttonStyles.locked}
              >
                <CloseIcon />
            ...
            <div
              styleName={opponent2Styles}
              onClick={() =>
                isOpponent2Selected
                  ? this.handleRemoveToBetSlip(opponent2.id)
            ...
            >
              ...
              <span styleName="group right">
                ...
                {opponent2.status !== "stable" && (
                  <div styleName={`arrow ${opponent2.status}`} />
                )}
              </span>
            </div>
          </div>
