...
class TableDefault extends Component {
  ...
  createSkeletonRows = () => {
    ...
    for (let i = 0; i < 10; i++) {
      tabs.push(
        <div styleName="skeleton-row" key={uniqueId("skeleton-row-")}>
          <Skeleton height={30} />
        </div>,
      );
    }
  ...
  renderGameColumn(row, background) {
    return (
      <div styleName="image">
        ...
        >
          <img styleName="image-icon" src={row.image_url} alt={row.name} />
        </div>
                    ...
                    {fields.map((field, index) => {
                      ...
                      if (field.dependentColor) {
                        return (
                          <td
                            styleName={styles}
                            data-label={titles[index]}
                            key={uniqueId("table-default-column-")}
                          >
                            {onTableDetails ? (
                              <a
                                href="/"
                                onClick={onTableDetails.bind(this, {
                                  titles,
                                  fields,
                                  row,
                                  tag,
                              ...
                              >
                                <Typography
                                  variant="x-small-body"
                                  color={
                                    row[field.condition] ? "green" : "grey"
                                  }
                                >
                                  {row[field.value]}
                                </Typography>
                                {this.getCurrencyImage(
                                  field.currency,
                                  row["currency"],
                                )}
                              </a>
                        ...
                        const background = row[field.value].hasOwnProperty(
                          "background_url",
                        )
                          ? row[field.value].background_url
                          : null;
                        return (
                          <td
                            styleName={styles}
