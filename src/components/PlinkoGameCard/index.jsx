...
class PlinkoGameCard extends React.Component {
      ...
      let checkParticleStatus = setInterval(() => {
        this.engine.world.bodies.forEach(dt => {
          if (
            dt.label === "particle" &&
            dt.position.y >
              this.state.CANVAS_HEIGHT - 5 - this.state.particleradius
          ) {
  ...
  _createPlinkos = (ROWS, r) => {
    let ROW_SPACING =
      (this.state.CANVAS_HEIGHT / ROWS) * this.state.ROW_ADJUSTMENT;
    let COL_SPACING =
      (this.state.CANVAS_WIDTH / (ROWS + 2)) * this.state.COL_ADJUSTMENT;
    ...
    for (row; row < ROWS + 2; row++) {
      let differ =
        (ROWS + 2 - row) * HORIZONTAL_OFFSET +
        this.state.COL_ADJUSTMENT * 90 +
        4;
  ...
  renderResult = () => {
    ...
    return <h6>{result.multiplier}x</h6>;
  };
          ...
          <Col span={18} push={6} gutter={16}>
            <div styleName="canvas-container">
              ...
              <Pegs
                ...
                peg10={peg10}
              />
              <div styleName="result">{this.renderResult()}</div>
            </div>
          </Col>
        </Row>
      </div>
