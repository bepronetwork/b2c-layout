...
class InfiniteCarousel extends Component {
    ...
    if (breakpoints.length > 0) {
      ...
      breakpoints.forEach((element, index) => {
        ...
        media(query, () => {
          ...
          const slideUniqueIds = children.map(() => uniqid("slide-")); // eslint-disable-line  no-unused-vars
          this.setState(
            {
  ...
  getFormatedChildren = (children, lazyLoadedList, visibleSlideList) =>
    Children.map(children, (child, index) => {
      ...
      if (!settings.lazyLoad || lazyLoadedList.indexOf(index) >= 0) {
        return (
          <li
            styleName="InfiniteCarouselSlide"
  ...
  init = () => {
    ...
    if (breakpoints.length > 0) {
      ...
      const slideUniqueIds = newChildren.map(() => uniqid("slide-")); // eslint-disable-line  no-unused-vars
      this.setState(
        {
  ...
  render() {
    const { scrollOnDevice, pagingSeparator, name, title } = this.props;
    ...
    if (settings.paging && !singlePage && !hasScrollOnDevice) {
      dots = (
        <span data-testid={`${name}-paging`} styleName="InfiniteCarouselPaging">
    ...
    return (
      <div>
        <div styleName="InfiniteCarouselTitle">
        ...
        <div
          id={name}
          data-testid={name}
          styleName="InfiniteCarousel"
          onFocus={this.onMouseOver}
          onMouseEnter={this.onMouseEnter}
          onMouseLeave={this.onMouseLeave}
          onMouseOver={this.onMouseOver}
        >
          <div styleName="InfiniteCarouselFrame" ref={this.storeFrameRef}>
            <ul
              styleName={trackClassName}
              style={trackStyles}
