...
class ActionBox extends React.Component {
  ...
  render() {
    ...
    return (
      <button
        disabled={completed || onLoading || disabled}
        onClick={this.onClick}
        styleName={`container-root ${completed ? "picked" : ""} ${
          completed || onLoading || disabled ? "nohover" : ""
        } ${onLoading ? "onLoading" : ""}${alertCondition ? "alert" : ""}`}
      >
        <Row>
          <Col xs={3} md={3}>
