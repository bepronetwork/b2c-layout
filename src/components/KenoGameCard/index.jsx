...
class KenoGameCard extends Component {
  ...
  async projectData(props) {
    ...
    this.setState({ ...this.state, edge: props.game.edge, result });
  }
  ...
  renderPayouts() {
    ...
    for (let index = 0; index <= numberOfCardsPicked; index++) {
      ...
      payouts.push(
        <div styleName="payout">
          <Typography
            variant={"x-small-body"}
            color={"grey"}
          >{`${this.formatPayout(payout.toFixed(2))}x`}</Typography>
        </div>
      );
    }

    return (
      <div styleName={`payouts payouts-${numberOfCardsPicked}`}>{payouts}</div>
    );
  }
  ...
  renderHits() {
    ...
    return (
      <div>
        <div styleName={`hits hits-${numberOfCardsPicked}`}>{hits}</div>
        {showChance == true ? (
