...
class MatchPage extends Component {
  ...
  updateBetSlip = (match, id) => {
    ...
    const ids =
      betSlip && _.isArray(betSlip)
        ? betSlip.map((bet) => bet.externalMatchId)
        : [];
    ...
    if (ids.includes(id)) {
      ...
      const betSlipArr = newBetSlip.map((bet) => {
        if (bet.externalMatchId === match.id && !_.isEmpty(match.odds)) {
          const opponent = match.odds[bet.type].find(
            (opponent) => opponent.participant_id === bet.id,
          );
          const marketActive = match.market
