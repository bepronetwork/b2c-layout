...
class Esports extends Component {
  ...
  projectData = async () => {
    ...
    const hasHighlight =
      esportsScrenner.title ||
      esportsScrenner.subtitle ||
      esportsScrenner.button_text
        ? true
        : false;
  ...
  handlerSerieFilterClick = async (serieFilter) => {
    ...
    const matches =
      serieFilter.length > 0
        ? await getMatchesBySeries({ serie_id: serieFilter, status })
        : await getMatches({ status });
        ...
        <div styleName="button">
          {highlight.button_text && highlight.link_url ? (
            <Button
              ...
              theme="action"
            >
              <Typography color={"fixedwhite"} variant={"small-body"}>
  ...
  renderSlides() {
    ...
    slides.slice(0, 3).map((match) => {
      ...
      slidesElements.push(
        <div
          styleName="element"
          onClick={() => this.goToMatch(match.slug, match.id)}
        >
            ...
            <div styleName="tour">
              ...
              <div>
                <Typography
                  ...
                  color={"white"}
                >{`${match.league.name} - ${match.serie.full_name}`}</Typography>
              </div>
            </div>
            <div styleName="group-left">
              {!this.isActiveMarket(match) && (
            ...
            <div>
              ...
              <div styleName="all-matches">
                <Button
                  size={"x-small"}
                  theme="primary"
                  onClick={() => this.handlerAllMatchesClick()}
                >
