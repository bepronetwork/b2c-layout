...
class ChatPage extends React.Component {
  ...
  handleScroll = async () => {
    ...
    if (isGoDownVisible === false && !this.isInViewport(this.messagesEnd)) {
      ...
      this.isInViewport(this.messagesEnd)
    ) {
  ...
  sendMessage = async (e) => {
    ...
    if (_.isEmpty(this.props.profile)) {
      await store.dispatch(
        setMessageNotification(CopyText.Errors.en.CHAT_USER_NOT_LOGGED),
      );
    }
    try {
  ...
  createSkeletonMessages = () => {
    ...
    for (let i = 0; i < 150; i++) {
      messages.push(
        <SkeletonTheme
          key={uniqueId("skeleton-messages-")}
          color={color}
          highlightColor={highlightColor}
        >
          <div styleName="message-box" key={i} style={{ opacity: "0.5" }}>
            <div styleName="info">
              <Skeleton width={100} />
            </div>
            <div styleName={"info-message-container"}>
