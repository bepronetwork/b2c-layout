    ...
    switch (serverJSON[key]) {
      ...
      case "colors": {
        ...
        var serverColors = Object.keys(serverJSON.value).map((k) => {
          ...
          var hslColor =
            "hsl(" +
            parseInt(hslColorEl.h) +
            "," +
            parseInt(hslColorEl.s) +
            "%," +
            parseInt(hslColorEl.l) +
            "%)";
          ...
          fibonacci.map((f) => {
            ...
            var color = {
              object: "variable",
              key: serverJSON.value[k].type + "-" + f,
              value: hslToLightOrDark(
                hslColorEl.h,
                hslColorEl.s,
                hslColorEl.l,
                theme === "dark" ? f : f * -1
          ...
          var color = {
            object: "variable",
            key: serverJSON.value[k].type + "-dark-3",
            value: hslToLightOrDark(
              hslColorEl.h,
              hslColorEl.s,
              hslColorEl.l,
...
async function generateHeadElements() {
  ...
  if (appInfo.hasOwnProperty("typography")) {
    ...
    if (url) {
      const fontLink =
        '{ "node": "element", "tag": "link", "attr": { "href": "' +
        url +
        '", "rel": "stylesheet" } }';
...
function generateNavBarName(html) {
  /* Get NavBar Name */
  const { name } = appInfo;
  const description = appInfo.hasOwnProperty("description")
    ? appInfo.description
    : null;
  const navBarName = `${name} - ${description}`;
  let titleIndex = html.child[0].child[1].child.findIndex(
    (c) => c.tag && c.tag.toLowerCase() == "title"
  );
...
async function setFontColors() {
  ...
  fs.writeFile(
    "src/styles/serverVariables.css",
    JSONtoSASS(objectServerInfo) + "\n\n",
    () => {
...
async function saveSkinStyles() {
  ...
  skinsPath.map((s) => {
    let file = fs.readFileSync(
      s.path + "/skins/index-" + skinType + ".css",
      "utf8"
    );
